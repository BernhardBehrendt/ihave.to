 ...
 *
 * @param {String} determines where to connect
 */
Connection = function () {
...
Connection.prototype.connect = function (board) {
    if (typeof (board) != CONF.PROPS.STRING.UD)
        this.__board = board;
    else if (this.__board === null)
        return false;
...
Connection.prototype.initBroadcast = function (sVerifier) {
    ...
    this.socket.on('bc-' + sVerifier, function (data) {
        ...
        if (typeof (oDiff.PRIVATE) != CONF.PROPS.STRING.UD && typeof (oDiff.PRIVATE.SCREENS) != CONF.PROPS.STRING.UD) {
            for (var sScreenName in CONF.BOARD.PRIVATE.SCREENS)
                if (!CONF.BOARD.PRIVATE.SCREENS[sScreenName])
                    delete CONF.BOARD.PRIVATE.SCREENS[sScreenName];

            // Detect if the change affected the active screen
            if (typeof (oDiff.PRIVATE.SCREENS[sActiveScreen]) != CONF.PROPS.STRING.UD) {
                // Not good implementation (everything is recreated)
                if (typeof (CONF.BOARD.PRIVATE.SCREENS[sActiveScreen]) != CONF.PROPS.STRING.UD) {
                    var bUpdated = false;

                    // Update the existing posts
                    for (var iTimeEntry in oDiff.PRIVATE.SCREENS[sActiveScreen].POSTS) {
                        ...
                        function updateScreen(oItem) {
                            ...
                            if (!( oItem instanceof Array)) {
                                if (typeof (oItem.TGT) != CONF.PROPS.STRING.UD) {
                                    ...
                                    if (oTarget.length == 1) {
                                        ...
                                        for (var sUser in CONF.BOARD.USERS) {
                                            if (CONF.BOARD.USERS[sUser] == oItem.BY)
                                                break;
                                        }
                    ...
                    if (!bUpdated) {
                        var oScreen = CONF.BOARD.PRIVATE.SCREENS[sActiveScreen];
                    ...
                    for (var sFirstScreen in CONF.BOARD.PRIVATE.SCREENS)
                        break;
        ...
        if (typeof (oDiff.USERS) != CONF.PROPS.STRING.UD) {
            for (var sUserName in oDiff.USERS)
                break;
...
Connection.prototype.encrypt = function (diffdata) {
    ...
    if (typeof (diffInternal) == CONF.PROPS.STRING.UD)
            ...
            buttons: {
                confirm: {
                    action: function (e) {
                        ...
                        if (typeof (CONF.BOARD.USERS[sName]) == CONF.PROPS.STRING.UD) {
                            var iIdUser = parseInt(Object.keys(CONF.BOARD.USERS).length);
        ...
        // If user is known set User id and send welcome message
            ...
            for (var sStoredName in CONF.BOARD.USERS) {
                if (sUserName == sStoredName) {
                    ...
                    setTimeout(function () {
                        showMessage('WELCOME_BACK'.translate() + ' ' + sUserName);
                    }, 5000);
                    break;
                }
            }
